...
# Crear un evento para señalar cuando los hilos deben detenerse
...
def generate_script(titulo, contexto, palabraclave, idioma, duracion):
    # Generate a JSON file and act as an educational tutorial creator for YouTube, creating a script with as many parts as necessary that transform common questions such as "How to...?" in detailed and captivating explanations about [{titulo}], with clear and consistent descriptions in each part, and enriched with an educational and engaging narrative to immerse the audience in learning.
    
    # prompt = f"""
    #     Goal:

    #     Generate a JSON file and act as an educational video creator for YouTube Shorts, creating a script with as many parts as necessary that transform common questions such as "How to...?" in short and captivating explanations about this main topic ["{titulo}"], with clear and consistent descriptions in each part, and enriched with an educational and engaging narrative to immerse the audience in learning. The script should also incorporate the following elements without mention them literaly in narration, that are used for viral success on YouTube Shorts:
    ...
    #     -- video_keyword: in English, specify one keyword that represents the main topic ["{titulo}"]. It is very important that the keywords are in english.
    ...
    if idioma.lower() in ["español", "espanol", "spanish"]:
        video_keyword = ""
        if (palabraclave):
            video_keyword = f"Literalmente usa ésta en cada escena: [{palabraclave}]"
        else:
            video_keyword = f'en Inglés, especificar 2 palabras claves que representen el tema principal: ["{titulo}"]. Repite la primera palabras clave en todos los elementos de "scene_dialog" y agrega una adicional. Es muy importante que sean 2 palabras claves en idioma inglés y que sea una cadena de texto.'

        prompt = f"""
            Objetivo:

            Actúa como un experto creador de videos para YouTube, TikTok, Instagram, Facebook y LinkedIn con una duración de {duracion} segundos o menos, creando un guión que transformen expresiones en contenidos cautivadores sobre este tema principal: ["{titulo}"] y el contexto del tema principal: ["{contexto}"], con descripciones claras y consistentes en cada parte, y enriquecidas con una narrativa educativa y atractiva para sumergir a la audiencia en el aprendizaje. El guión también debe incorporar los siguientes elementos sin mencionarlos literalmente en la narración, los cuales se utilizan para el éxito viral en YouTube, TikToks, Instagram, Facebook y LinkedIn:

            - Utiliza el estilo y estrategia de MrBeast y Jenny Hoyos para la generación de títulos virales.
            - Comenzar con algo que sirva de gancho, que capte inmediatamente la atención. Ejemplos: MrBeast: ["¡Aprendí 5 Nuevas Habilidades en 24 Horas! - Desafío de Aprendizaje Acelerado", "¡Memoricé el Alfabeto en Lenguaje de Señas en 60 Segundos! - Desafío Educativo de Inclusión", "¡Exploré 10 Hechos Científicos Asombrosos en 1 Minuto! - Educación Rápida y Divertida"] Jenny Hoyos: ["¡Cociné 3 Platos Saludables! - Desafío Creativo", "¡Transformé Mi Espacio de Estudio! - Renovación Ecológica", "¡Hice Ejercicio con Objetos Caseros! - Fitness Innovador"]. (No mencionar este elemento literalmente en la narración)
            ...
            thumbnail_prompt_dalle: en Inglés, desarrollar un mensaje para DALL·E que describa el tema principal: ["{titulo}"] y el contexto del tema principal: ["{contexto}"] con detalle y precisión.
        ...
        """
    elif idioma.lower() in ["inglés", "ingles", "english"]:
        video_keyword = ""
        if (palabraclave):
            video_keyword = f"Literally use this in every scene: [{palabraclave}]"
        else:
            video_keyword = f'Specify 2 keywords that represent the main topic: ["{titulo}"]. Repeat the first keyword in all elements of "scene_dialog" and add an additional one. It is very important that they are 2 keywords in a text string.'

        prompt = f"""
            Objective:

            Act as an expert video creator for YouTube, TikTok, Instagram, Facebook, and LinkedIn, crafting a script for videos lasting {duracion} seconds or less. Transform expressions into captivating content on this main topic: ["{titulo}"] and the context of the main topic: ["{contexto}"], with clear and consistent descriptions in each part, enriched with an educational and engaging narrative to immerse the audience in learning. The script should also incorporate the following elements without explicitly mentioning them in the narration, which are used for viral success on YouTube, TikToks, Instagram, Facebook, and LinkedIn:

            - Use the style and strategy of MrBeast and Jenny Hoyos for generating viral titles.
            - Start with something that serves as a hook, immediately capturing attention. Examples: MrBeast: ["I Learned 5 New Skills in 24 Hours! - Accelerated Learning Challenge", "I Memorized the Sign Language Alphabet in 60 Seconds! - Inclusive Educational Challenge", "I Explored 10 Amazing Scientific Facts in 1 Minute! - Quick and Fun Education"] Jenny Hoyos: ["I Cooked 3 Healthy Dishes! - Creative Challenge", "I Transformed My Study Space! - Ecological Renovation", "I Exercised with Household Items! - Innovative Fitness"]. (Do not mention this element literally in the narration)
            ...
            - Address trending topics, such as current events, viral challenges, popular memes, or high-interest topics. (Do not mention this element literally in the narration)
            ...
            - Conclude with a direct message for viewer action, such as sharing, liking, commenting, or participating in a specific activity, thus promoting video engagement and dissemination. (Do not mention this element literally in the narration)
            - Do not personalize the script as if it were presented by a host, as this video will be fully produced by an AI.
            - If you are going to talk about Artificial Intelligence, do not use the acronym "AI", use the full name.
            ...
            thumbnail_prompt_dalle: Develop a message for DALL·E that describes the main topic: ["{titulo}"] and the context of the main topic: ["{contexto}"] with detail and precision.
...
def generate_keywords(titulo):

    prompt = f"""
        Act as an expert finding keywords in outlines like this: [{titulo}]. Then generate the main keyword in English that help to find videos related to the outline.
    """
...
def generate_image_prompt(prompt, carpeta, orientacion):
    ...
    escena = f"""Create an image with orientation {orientacion} and rendered in a realistic and cinematic three-dimensional style, inspired by the visual quality and atmosphere of modern films. This image should appeal to a wide audience, with expressive characters with clear definition of their body parts and detailed environments that capture the essence of a modern world. The focus should be on creating a clear and engaging presentation, conveying the story in a visually captivating and easy-to-understand manner, without replicating specific characters from these movies and avoiding traditional clipart or cartoon styles, for a YouTube Shorts video using this elements:\n\n
        [STEP: {prompt}]\n\n
    """
...
def text_to_speech_polly(text, output_filename, idioma, voz):
    
    print("text_to_speech_polly - voz:", voz)
...
def procesar_scene(scene, carpeta, orientacion, idioma, voz, fuentevideos, palabraclave, intervalo):
    global CONTADOR_ESCENAS, VIDEOS
    ...
    videos = []
    if (palabraclave and len(VIDEOS) == 0):
        if (fuentevideos == "storyblocks"):
            videos = search_storyblocks_videos(api_key, secret_key, expires, user_id, project_id, single_text_with_space)
        elif (fuentevideos == "pexels"):
            videos = search_pexels_video(video_keyword[0], orientacion)
        elif (fuentevideos == "envato"):
            videos = search_envato_video(single_text_with_space, orientacion)
        elif (fuentevideos == "videospropios"):
            videos = search_pexels_video(video_keyword[0], orientacion)
        else:
            videos = search_pexels_video(video_keyword[0], orientacion)
        VIDEOS = videos
    elif (not palabraclave):
        if (fuentevideos == "storyblocks"):
            videos = search_storyblocks_videos(api_key, secret_key, expires, user_id, project_id, single_text_with_space)
        elif (fuentevideos == "pexels"):
            videos = search_pexels_video(video_keyword[0], orientacion)
    ...
    print("# de videos, VIDEOS y origen A:", len(videos), len(VIDEOS), fuentevideos)

    if (len(videos) == 0):
        videos = search_pexels_video(single_text_with_space, orientacion)
        print("# de videos y origen B:", len(videos), fuentevideos)
    
    audio = text_to_speech_polly(scene["narration"], output_filename, idioma, voz)
    imagen = "" #generate_image_prompt(scene["prompt_dalle"], carpeta, "horizontal")
    duracion_audio = obtener_duracion_audio(output_filename)
    clip = generar_clips(videos, duracion_audio, orientacion, intervalo)
...
def generar_clips_original(videos, duracion_audio):
    ...
    while tiempo_actual < duracion_audio:
        print("generar_clips_original -> videos[indice_video]:", videos[indice_video])
...
def generar_clips(videos, duracion_audio, orientacion, intervalo): #ROMG ROMG
    ...
    while tiempo_actual < duracion_audio:
        ...
        clip = VideoFileClip(local_filename).subclip(0, intervalo)
        #clip = VideoFileClip(videos[indice_video]).subclip(0, intervalo)
...
# Función principal
def main(titulo, contexto, musica, palabraclave, idioma, orientacion, duracion, voz, fuentevideos, intervalo, stop_event = threading.Event()):
    global CONTADOR_ESCENAS
    ...
    print('# título español:', feeling or titulo or titulo)

    if stop_event.is_set():
        print("SALIDA PREMATURA 1")
        exit()

    script = generate_script(titulo, contexto, palabraclave, idioma, duracion)
    ...
    print(script)
    print("============================== FINAL GUION ===================================")
    # keywords = generate_keywords(titulo)
    ...
    for dialog in script_ia["scene_dialog"]:
        CONTADOR_ESCENAS += 1
        if stop_event.is_set():
            print("SALIDA PREMATURA 3")
            exit()
        
        elementos = procesar_scene(dialog, carpeta_elementos, orientacion, idioma, voz, fuentevideos, palabraclave, intervalo)
...
@appflask.route('/mainpost', methods=['POST'])
def mainpost():
    ...
    voz = data["voz"]
    fuentevideos = data["fuentevideos"]
    ...
    # Iniciar un nuevo hilo para ejecutar la función main
    threading.Thread(target=main, args=(titulo, contexto, musica, palabraclave, idioma, orientacion, duracion, voz, fuentevideos, intervalo, stop_event)).start()

    return jsonify({"message": "Parameters received, video creation in process"}), 202



@appflask.route('/stop', methods=['POST'])
def stop():
    stop_event.set()  # Señalar a los hilos que deben detenerse
    return jsonify({"message": "Stop signal sent"}), 200


@appflask.route('/videos', methods=['GET'])
def list_videos():
    # Obtener la lista de archivos mp4 y sus tiempos de modificación
    videos = [(f, os.path.getmtime(os.path.join(VIDEOS_FOLDER, f))) for f in os.listdir(VIDEOS_FOLDER) if f.endswith('.mp4')]
    # Ordenar los archivos por tiempo de modificación en orden descendente
    videos.sort(key=lambda x: x[1], reverse=True)
    # Preparar la lista de archivos con sus fechas de modificación
    video_files = [{"filename": f[0], "modified": datetime.fromtimestamp(f[1]).strftime('%Y-%m-%d %H:%M:%S')} for f in videos]
    return jsonify(video_files)


@appflask.route('/videos/<filename>')
def get_video(filename):
    return send_from_directory(VIDEOS_FOLDER, filename)


@appflask.route('/videos/<filename>', methods=['DELETE'])
...
if __name__ == "__main__":
    # Verificar si se están pasando argumentos por la línea de comandos
    if len(sys.argv) > 1:
        ...
        parser.add_argument("contexto", help="Introduce un contexto para el video.")
        ...
        parser.add_argument("voz", help="Introduce la voz del locutor.")
        parser.add_argument("fuentevideos", help="Introduce la fuente de los videos.")
        parser.add_argument("intervalo", help="Introduce el intérvalo de segundos de los videos.")
        args = parser.parse_args()

        print("\n======================== args:\n", args, "\n========================\n")

        main(args.titulo, args.contexto, args.musica, args.palabraclave, args.idioma, args.orientacion, int(args.duracion), args.voz, args.fuentevideos, int(args.intervalo))
