...
def generate_script(outline):
    ...
    prompt = f"""
        ...
        Gancho Inicial Impactante: Comenzar con algo que capte inmediatamente la atención. Ejemplos: "¿La IA compone una canción en 1 minuto: éxito o fracaso?", "Chatbot de IA organiza mi agenda: ¿Mejor que un asistente?", "Desafío de IA: ¿Quién edita fotos mejor, humano o máquina?", "IA para ahorrar: ¿Realmente reduce mis gastos mensuales?". (No mencionar este elemento literalmente en la narración)
        Contenido Breve y Conciso: Limitar la duración a 34 segundos o menos, asegurando que el contenido sea rápido y fácil de consumir.
        Contenido Relevante y Actual: Abordar temas de tendencia, como eventos actuales, desafíos virales, memes populares o temas de gran interés. (No mencionar este elemento literalmente en la narración)
        Elemento de Sorpresa o Inesperado: Incluir un giro o evento inesperado que genere emoción o sorpresa, como un cambio abrupto en la narrativa o una reacción poco común. (No mencionar este elemento literalmente en la narración)
        Llamado a la Acción Claro: Concluir con un mensaje directo para la acción del espectador, como compartir, dar me gusta, comentar o participar en una actividad específica, promoviendo así la participación y difusión del video. (No mencionar este elemento literalmente en la narración)
        ...
        El JSON debe contener un objeto en un arreglo de un elemento, representando el título del tutorial, colocado en un campo llamado "script_video_ia".
        ...
        -- narration: En español, el texto narrativo real hablado por el narrador, que debe describir el título, los pasos y las acciones en detalle, en un estilo consistente con el enfoque general del tutorial.
        ...
        -- video_keyword: en inglés, especificar una palabra clave que represente el tema principal ["{outline}"]. Es muy importante que las palabras clave estén en inglés.
        
        Consistencia en las descripciones y definiciones de título y subtítulo:

        Asegurar que el título y subtítulo sean idénticos en cada objeto del tutorial, al texto proporcionado en el tema principal.
...
def generate_keywords(outline):
    ...
    response = client.chat.completions.create(
        messages=[
            {"role": "system", "content": "You are a useful virtual assistant for story telling about tutorials. Just return the 3 keywords divided by spaces."},
            {"role": "user", "content": prompt}
        ],
        # model="gpt-3.5-turbo-16k",
        model="gpt-4-1106-preview",
...
def text_to_speech_polly(text, output_filename):
    ...
    voice="Lupe" #es-MX: Mia, Andrés | es-US: Miguel, Lupe, Penelope 
...
def create_video_with_background_video(elementos_partes, output_filename, background_music):
    ...
    if 'narration' in elementos:
        ...
        for index, text in enumerate(elementos['narration']):
            ...
            for palabra, duracion in tiempos_palabras:
                ...
                text_clip = TextClip(palabra, fontsize=50, color='white', method='caption', align='center').set_duration(duracion).set_pos('center').set_start(current_time) #, size=video.size
                txt_size = text_clip.size
                bg_size = (txt_size[0] + 10, txt_size[1] + 10)
                bg_clip = ColorClip(size=bg_size, color=(0, 0, 0), duration=duracion).set_opacity(0.5).set_position(('center', 'center')).set_start(current_time)
        ...
        video = mpe.CompositeVideoClip([video] + narration_clips)
...
def procesar_scene(scene, carpeta):
    ...
    videos = search_pexels_video(scene["video_keyword"])
    print("# de videos:", len(videos))
...
def main(outline, musica):
    ...
    create_video_with_background_video(elementos_partes, video_filename, background_music)
