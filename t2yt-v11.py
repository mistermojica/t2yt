...
def generate_script(verse_text):
    ...
    prompt = f"""
        ...
        Generate a JSON file and act as an educational video creator for YouTube Shorts, creating a script with as many parts as necessary that transform common questions such as "How to...?" in short and captivating explanations about [{verse_text}], with clear and consistent descriptions in each part, and enriched with an educational and engaging narrative to immerse the audience in learning. The script should also incorporate the following elements for viral success on YouTube Shorts:

        1. Striking Initial Hook: Start with something that immediately captures attention. Examples: "AI composes a song in 1 minute: Hit or flop?", "AI chatbot organizes my schedule: Better than an assistant?", "AI Challenge: Who edits photos better, human or machine?", "AI for saving: Does it really cut my monthly expenses?".
        2. Brief and Concise Content: Limit the duration to 34 seconds or less, ensuring the content is quick and easy to consume.
        3. Surprise or Unexpected Element: Include a twist or unexpected event that generates excitement or surprise, such as an abrupt change in the narrative or an uncommon reaction.
        4. Relevant and Current Content: Address trending topics, such as current events, viral challenges, popular memes, or topics of great interest.
        5. Clear Call to Action: Conclude with a direct message for viewer action, such as sharing, liking, commenting, or engaging in a specific activity, thus promoting participation and spreading the video.
        ...
        - video_keywords: in English, specify 3 keywords that represents each title. It is very important that the keywords are in english.

        - scene_dialog: A narrative entry, corresponding to a title. These entry is delivered by a single narrator in a voice-over style, done with educational language that is easy for anyone to understand, essential to convey the story and context of the title. The narrative entry must include:
        ...
        - Ensure that the title and subtitle are identical in each object of the tutorial, to the text provided in the outline.
...
def generate_keywords(outline):
    ...
    prompt = f"""
        Act as an expert finding keywords in outlines like this: [{outline}]. Then generate the main 3 keywords in English that help to find videos related to the outline.
    """
...
def create_video_with_background_video(images, background_videos, audio_files, background_music, output_filename):
