...
def generate_script(outline, palabraclave):
    ...
    prompt = f"""
        ...
        Actua como un creador de videos educativos para YouTube Shorts de 54 segundos o menos, creando un guión que transformen expresiones en contenidos cortos y cautivadores sobre este tema principal ["{outline}"], con descripciones claras y consistentes en cada parte, y enriquecidas con una narrativa educativa y atractiva para sumergir a la audiencia en el aprendizaje. El guion también debe incorporar los siguientes elementos sin mencionarlos literalmente en la narración, que se usan para el éxito viral en YouTube Shorts:
...
def generate_image_prompt(prompt, carpeta, orientation="vertical"):
    ...
    # escena = f"""Create a image with orientation {orientation} and rendered in a vibrant and colorful three-dimensional style, inspired by the visual quality and atmosphere of 3D animated movies like 'Toy Story' and 'Shrek'. This image should appeal to a wide audience, with expressive characters and friendly environments that capture the essence of a lively animated world. The focus should be on creating a clear and cheerful presentation, conveying the story in a visually enchanting and easy-to-understand manner, without replicating specific characters from these movies and avoiding traditional clipart or cartoon styles, for a YouTube Shorts video using this elements:\n\n
    # [STEP: {prompt}]\n\n
    # """

    escena = f"""Create an image with orientation {orientation} and rendered in a realistic and cinematic three-dimensional style, inspired by the visual quality and atmosphere of epic historical films. This image should appeal to a wide audience, with expressive characters and detailed environments that capture the essence of a dramatic ancient Egyptian world. The focus should be on creating a clear and engaging presentation, conveying the story in a visually captivating and easy-to-understand manner, without replicating specific characters from these movies and avoiding traditional clipart or cartoon styles, for a YouTube Shorts video using this elements:\n\n
        [STEP: {prompt}]\n\n
    """
...
def create_video_with_background_video(elementos_partes, titulo, output_filename, background_music, thumbnail_background_filename):
    ...
    if 'narration' in elementos:
        ...
        thumbnail_filename = crear_thumbnail(titulo.replace(" ", "\n"), thumbnail_background_filename, "")

        # Convertir la imagen en un ImageClip
        thumbnail_clip_temp = mpe.ImageClip(thumbnail_filename).set_duration(1) #.set_start(0)  # Duración de 5 segundos, ajusta según necesites
...
def procesar_scene(scene, carpeta):
    ...
    videos_pexels = search_pexels_video(video_keyword[0])
    ...
    if len(videos) == 0:
        videos = search_pexels_video(single_text_with_space)
...
def generar_clips(videos, duracion_audio):
    ...
    while tiempo_actual < duracion_audio:
        clip = VideoFileClip(videos[indice_video]).subclip(0, segundos_videos)
        ...
        if original_width > original_height:
            # Calcular el nuevo ancho para mantener la relación de aspecto
...
def crear_thumbnail(titulo, ruta_fondo, ruta_persona):
    ...
    font = ImageFont.truetype('./fonts/bebas_neue/BebasNeue-Regular.ttf', 110)
...
# Función principal
def main(outline, musica, palabraclave):
    global CONTADOR_ESCENAS
    ...
    script = generate_script(outline, palabraclave)
    ...
    thumbnail_filename = generate_image_prompt(thumbnail_prompt_dalle, "miniaturas", "vertical")
    ...
    create_video_with_background_video(elementos_partes, titulo, video_filename, background_music, thumbnail_filename)
...
def search_storyblocks_videos(api_key, secret_key, expires, user_id, project_id, keywords):
    ...
    for i in range(len(keywords_list), 0, -1):
        ...
        # print("===============================================================================================")
        # print("search_storyblocks_videos -> video_results:", video_results)
        # print("===============================================================================================")
...
if __name__ == "__main__": 
    ...
    main(args.titulo or subject, args.musica, args.palabraclave)
    # main(titulo)
