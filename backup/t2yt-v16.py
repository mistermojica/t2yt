...
def generate_script(outline, palabraclave, idioma):
    ...
    prompt = f"""
        ...
        -- narration: En idioma [{idioma}], el texto narrativo real hablado por el narrador, que debe describir el título, los pasos y las acciones en detalle, en un estilo consistente con el enfoque general del video.
...
def generate_image_prompt(prompt, carpeta, orientacion):
    ...
    escena = f"""Create an image with orientation {orientacion} and rendered in a realistic and cinematic three-dimensional style, inspired by the visual quality and atmosphere of epic historical films. This image should appeal to a wide audience, with expressive characters and detailed environments that capture the essence of a dramatic ancient Egyptian world. The focus should be on creating a clear and engaging presentation, conveying the story in a visually captivating and easy-to-understand manner, without replicating specific characters from these movies and avoiding traditional clipart or cartoon styles, for a YouTube Shorts video using this elements:\n\n
        [STEP: {prompt}]\n\n
    """
...
def text_to_speech_polly(text, output_filename):
    ...
    response = get_polly_response("generative", "Matthew", text, "90%")
...
def procesar_scene(scene, carpeta, orientacion):
    global CONTADOR_ESCENAS
...
# Función principal
def main(outline, musica, palabraclave, idioma, orientacion):
    global CONTADOR_ESCENAS
    ...
    script = generate_script(outline, palabraclave, idioma)
    ...
    for dialog in script_ia["scene_dialog"]:
        CONTADOR_ESCENAS += 1
        elementos = procesar_scene(dialog, carpeta_elementos, orientacion)
...
if __name__ == "__main__":
    ...
    main(args.titulo or subject, args.musica, args.palabraclave, args.idioma, args.orientacion)
    # main(titulo)
