...
def generate_script(frase_central):
    ...
    feeling_start = generated_text.find("__CENTRO__:") + len("__CENTRO__:")

    if feeling_start != -1:
        feeling_end = generated_text.find("\n", feeling_start)
        feeling_value = generated_text[feeling_start:feeling_end].strip()
        generated_text = generated_text[:feeling_start - len("__CENTRO__:")] + generated_text[feeling_end:]
    else:
        feeling_value = "No encontrado"

    # print("Texto generado:", generated_text)
    # print("__CENTRO__:", feeling_value)

    result = {
        "generated_text": generated_text,
        "feeling_value": feeling_value.lower()
    }
...
def create_video_with_background_video(background_videos, audio_files, background_music, output_filename):
    ...
    for i, url in enumerate(background_videos):
        ...
        # output_filename = f'./videos/background_video_{i}_resized.mp4'
        download_video(url, input_filename)
        # resize_and_pad(input_filename, output_filename, 360, 640)
        video_filenames.append(input_filename)

    # Cargar y concatenar videos de fondo
    # video_clips = [mpe.VideoFileClip(f) for f in video_filenames]
    video_clips = [mpe.VideoFileClip(f) for f in video_filenames]

    # Recortar la duración de cada clip a 5 segundos
    for i in range(len(video_clips)):
        video_clips[i] = video_clips[i].subclip(0, 5)

    # Concatenar los clips de video recortados
    video = mpe.concatenate_videoclips(video_clips)

    # Cargar y combinar archivos de audio
    # audio = mpe.CompositeAudioClip([mpe.AudioFileClip(f) for f in audio_files])
    # Cargar y combinar archivos de audio
    voice_audio = mpe.CompositeAudioClip([mpe.AudioFileClip(f) for f in audio_files])
    music_audio = mpe.AudioFileClip(background_music)
